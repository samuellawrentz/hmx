...
$conf =
	$mm['arguments']['config']
	?? (getenv('XDG_CONFIG_HOME') ?: getenv('HOME').'/.config').'/hmx/config'
;
...
config($mm, 'map_dir',               getenv('HOME').'/maps');
config($mm, 'link_color',            "\033[38;5;33m");
config($mm, 'body_marker_color',     "\033[38;5;214m");
config($mm, 'overdue_color',         "\033[38;5;196m");
...
$keybindings[special_keys['back_space']] = 'go_back';
$keybindings["\010"] = 'go_back';

$keybindings[special_keys['ctrl_e']] = 'extract_to_map';
...
function list_to_map($lines, $root_id, $start_id)
{
	...
	foreach ($lines as $line)
		...
		{
			$stripped = ltrim($line);
			$sigil    = mb_substr($stripped,0,2);

			if ($sigil=='> ' || $stripped=='>')
			{
				if ($id > $start_id)
				{
					$text = mb_substr($stripped, $sigil=='> ' ? 2 : 1);
					$nodes[$id-1]['body'] =
						isset($nodes[$id-1]['body'])
						? $nodes[$id-1]['body']."\n".$text
						: $text
					;
				}
...
function map_to_list(&$mm, $id, $exclude_parent = false, $base = 0)
{
	if (!$exclude_parent)
	{
		...
		if (($mm['nodes'][$id]['body'] ?? '') !== '')
			foreach (explode("\n", $mm['nodes'][$id]['body']) as $line)
				$output .= str_repeat("\t",$base).'> '.$line.PHP_EOL;
	}
	else
		$output = '';
...
function map_node_count($filename)
{
	$count = 0;

	foreach (file($filename, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) as $line)
		if (mb_substr(ltrim($line),0,1) != '>')
			$count++;
...
function calculate_x_and_lh(&$mm, $id)
{
	...
	if ( mb_strlen($title) > width_tolerance * $max_width )
	{
		$lines = explode(PHP_EOL ,wordwrap($title ,$max_width, PHP_EOL));
	...
	{
		$mm['nodes'][$id]['w']  = mb_strwidth(trim($title));
		$mm['nodes'][$id]['wl'] = mb_strlen(trim($title));
		$mm['nodes'][$id]['lh'] = 1;
	}
...
function help()
{
	...
	echo "\nlist screen: j/k move, Enter open, n new, r rename, d delete, / filter, q quit\n";
...
function display(&$mm, $force_center = false)
{
	...
	$body = $mm['nodes'][$mm['active_node']]['body'] ?? '';
	$rows = $body === '' ? $mm['terminal_height'] : (int)($mm['terminal_height'] * 0.7);
	...
	for ( $y = 0 ; $y < $rows ; $y++ )
	{
		...
		// links and the body marker: skipped on the active rows, a colour reset would cut the highlight

		if ($y < $y1 || $y >= $y2)
		{
			// styling "[[task:uuid]]" links, red when overdue

			$line =
				preg_replace_callback
				(
					 '/\[\[task:([0-9a-z]{8})\]\]/'
					,fn($m) => (($mm['tasks'][$m[1]]['overdue'] ?? false) ? $mm['overdue_color'] : $mm['link_color']).$m[0].default_color
					,$line
				);
			...
			$line =
				...
				(
					 '(\[\[(?!task:)[^\]]*\]\])'
					,$mm['link_color'].'\\1'.default_color
					,$line
				);

			// styling the "…" body marker

			$line = str_replace('…', $mm['body_marker_color'].'…'.default_color, $line);
		}
	...
	// showing the body pane

	if ($body !== '')
	{
		$sep_row = $rows + 1;
		echo "\033[{$sep_row};1f".line_on.str_repeat('─', $mm['terminal_width']).line_off;

		$body_lines = explode("\n", $body);
		$last_row   = $mm['terminal_height'] - 1;
		for ($i = 0; $i < count($body_lines) && $sep_row + 1 + $i <= $last_row; $i++)
			echo "\033[".($sep_row + 1 + $i).";1f".mb_strimwidth($body_lines[$i], 0, $mm['terminal_width'], '');
	}
	...
	if (!empty($mm['stack']))
	{
		$names = array_map(fn($s) => basename($s['file'], '.hmm'), $mm['stack']);
		$names[] = basename($mm['filename'], '.hmm');

		echo "\033[1;1f".implode(' › ', $names).' ['.count($mm['stack']).']'.reset_color;
	}
...
{
	global $mm;
	$n = $mm['nodes'][$mm['active_node']];
	if (preg_match('/\[\[task:([0-9a-z]{8})\]\]/', $n['title'], $m))
		show_task($m[1]);
	else if (str_contains($n['title'].($n['body'] ?? ''), '[['))
		follow_link();
	else
		insert_new_sibling();
}

function show_task($uuid)
{
	global $mm;
	if (exec('command -v task') === '')
	{
		message($mm, 'task binary not found');
		return;
	}

	run_in_tty('task rc.context=none '.escapeshellarg($uuid).' info | '.(getenv('PAGER') ?: 'less'));
	display($mm);
}

function run_in_tty($cmd)
{
	echo "\033[?1049l\033[?25h";
	system('stty sane');
	passthru($cmd.(posix_isatty(STDOUT) ? ' >/dev/tty' : ''));
	enable_alternate_screen();
}

function node_text($mm, $node)
{
	$title =
		preg_replace_callback
		(
			 '/\[\[task:([0-9a-z]{8})\]\]/'
			,fn($m) => (($mm['tasks'][$m[1]]['done'] ?? false) ? '☑' : '☐').' '.$m[0]
			,$node['title']
		);
...
function tasks_from_export($json)
{
	$tasks = [];
	foreach (json_decode($json, true) ?: [] as $task)
	{
		$done = $task['status'] === 'completed';
		$tasks[substr($task['uuid'], 0, 8)] =
		[
			 'done'    => $done
			,'overdue' => isset($task['due']) && !$done && strtotime($task['due']) < time()
		];
	}
	return $tasks;
}

function load_tasks(&$mm)
{
	$uuids = [];
	foreach ($mm['nodes'] as $node)
		if (preg_match_all('/\[\[task:([0-9a-z]{8})\]\]/u', $node['title'] ?? '', $m))
			$uuids = array_merge($uuids, $m[1]);

	$mm['tasks'] = [];
	if (empty($uuids) || exec('command -v task') === '')
		return;

	exec('task rc.context=none rc.verbose=nothing '.implode(' ', array_unique($uuids)).' export 2>/dev/null', $out);
	$mm['tasks'] = tasks_from_export(implode('', $out));
}
...
function parse_link($text)
{
	...
	if (mb_substr($m[1], 0, 5) === 'task:')
		return null;

	$parts = explode('#', $m[1], 2);
	return ['map' => $parts[0], 'node' => $parts[1] ?? null];
}
...
function go_to_node($id)
{
	global $mm;
	$mm['active_node'] = $id;
	for ($p = $mm['nodes'][$id]['parent']; $p > 0; $p = $mm['nodes'][$p]['parent'])
		$mm['nodes'][$p]['collapsed'] = false;
}

function open_map($file, $node = null)
{
	global $mm;
	...
	[
		...
		,'viewport_top'  => $mm['viewport_top']
		,'viewport_left' => $mm['viewport_left']
	]);
	...
	{
		$id = null;
		foreach ($mm['nodes'] as $nid=>$nd)
			if ($nid != 0 && $nd['title'] === $node)
				{ $id = $nid; break; }

		if ($id === null)
			foreach ($mm['nodes'] as $nid=>$nd)
				if ($nid != 0 && mb_stripos($nd['title'], $node) !== false)
					{ $id = $nid; break; }

		if ($id === null)
			message($mm, "node not found: $node");
		else
			go_to_node($id);
	}
...
function follow_link()
{
	...
	$n = $mm['nodes'][$mm['active_node']];
	$link = parse_link($n['title']) ?? parse_link($n['body'] ?? '');

	if ($link === null)
		return;

	$file = $mm['map_dir'].'/'.$link['map'].'.hmm';

	if (!file_exists($file))
	{
		message($mm, "create {$link['map']}.hmm? [y/N]");
		...
		if (strtolower(substr($key,0,1)) !== 'y')
		{
			display($mm);
			return;
		}

		if (!is_dir($mm['map_dir']))
			mkdir($mm['map_dir'], 0755, true);

		file_put_contents($file, $link['map'].PHP_EOL);
	}

	open_map($file, $link['node']);
}
...
function go_back()
{
	...
	if (isset($mm['nodes'][$prev['active_node']]))
	{
		go_to_node($prev['active_node']);
		$mm['viewport_top']  = $prev['viewport_top'];
		$mm['viewport_left'] = $prev['viewport_left'];
	}
...
function extract_subtree($id, $key)
{
	global $mm;

	$file = $mm['map_dir'].'/'.$key.'.hmm';

	if (file_exists($file))
	{
		message($mm, "$key.hmm exists, pick another name");
		return false;
	}

	push_change($mm);

	$mm['nodes'][$id]['title'] = $key;
	file_put_contents($file, map_to_list($mm, $id));

	$mm['nodes'][$id]['title'] = "[[$key]]";
	$mm['nodes'][$id]['body'] = '';
	delete_children_vh($mm, $id);
	$mm['nodes'][$id]['is_leaf'] = true;
	$mm['nodes'][$id]['children'] = [];
...
function extract_to_map()
{
	...
	$slug = trim(mb_ereg_replace('[^a-z0-9]+', '-', mb_strtolower($mm['nodes'][$id]['title'])), '-');
	$default = basename($mm['filename'], '.hmm').'-'.$slug;

	$key = magic_readline($mm, $default);
...
function edit_body()
{
	...
	$tmp = tempnam(sys_get_temp_dir(), 'hmx').'.md';
	file_put_contents($tmp, $n['title']."\n\n".($n['body'] ?? '')."\n");

	run_in_tty((getenv('EDITOR') ?: 'vi').' '.escapeshellarg($tmp));

	$lines = explode("\n", rtrim(file_get_contents($tmp)));
	...
	push_change($mm);
	$mm['nodes'][$id]['title'] = $title;
	$mm['nodes'][$id]['body']  = $body;

	build_map($mm);
	display($mm);
}

function list_rows($filter)
{
	global $mm;
	$rows = [];
	foreach (glob($mm['map_dir'].'/*.hmm') as $file)
	{
		$name = basename($file, '.hmm');
		if ($filter !== '' && mb_stripos($name, $filter) === false)
			continue;
		$rows[] = ['name'=>$name, 'file'=>$file, 'count'=>map_node_count($file), 'mtime'=>filemtime($file)];
	}
	usort($rows, function($a, $b)
	{
		if (($a['name']==='inbox') !== ($b['name']==='inbox'))
			return $a['name']==='inbox' ? -1 : 1;
...
function list_screen()
{
	...
	while (true)
	{
		...
		$cursor = max(0, min($cursor, count($rows)-1));

		echo reset_page.reset_color;
		echo 'maps in '.$mm['map_dir'].($filter!=='' ? '  /'.$filter : '')."\n\n";
		if (empty($rows))
			echo ($filter!=='' ? "no match for /$filter" : 'no maps')."\n";

		foreach ($rows as $i=>$row)
		{
			$s = time() - $row['mtime'];
			$age = match(true) { $s<60 => $s.'s', $s<3600 => intdiv($s,60).'m', $s<86400 => intdiv($s,3600).'h', default => intdiv($s,86400).'d' };
			echo
				($i==$cursor ? $mm['active_node_color'] : '')
				.str_pad($row['name'], 32).str_pad($row['count'], 5, ' ', STR_PAD_LEFT)
				.'  '.str_pad($age, 4, ' ', STR_PAD_LEFT)
				.($i==$cursor ? reset_color : '')."\n"
			;
		}
		...
		if ($in=='j' || $in==special_keys['arr_down'])
			$cursor = min($cursor+1, count($rows)-1);
		elseif ($in=='k' || $in==special_keys['arr_up'])
			$cursor = max($cursor-1, 0);
		elseif ($in=='q' || $in==special_keys['ctrl_c'])
			return null;
		elseif ($in=='/')
		{
			$new = magic_readline($mm, $filter);
			if ($new !== false)
				$filter = $new;
			$cursor = 0;
		}
		elseif ($in==special_keys['enter'] && !empty($rows))
			return $rows[$cursor]['file'];
		elseif ($in==special_keys['enter'] && $filter !== '')
		{
			exec('grep -il '.escapeshellarg($filter).' '.escapeshellarg($mm['map_dir']).'/*.hmm', $hits);
			if (!empty($hits))
			{
				$mm['list_query'] = $filter;
				return $hits[0];
			}
			message($mm, 'no match');
		}
		elseif ($in=='n')
		{
			$name = magic_readline($mm, '');
			if ($name === false || $name === '')
				continue;
			$file = $mm['map_dir'].'/'.$name.'.hmm';
			if (file_exists($file))
				message($mm, "$name.hmm already exists");
			else
			{
				file_put_contents($file, $name.PHP_EOL);
				return $file;
			}
		}
		elseif ($in=='r' && !empty($rows))
		{
			$new = magic_readline($mm, $rows[$cursor]['name']);
			if ($new !== false && $new !== '' && $new !== $rows[$cursor]['name'])
				rename_map($rows[$cursor]['name'], $new);
		}
		elseif ($in=='d' && !empty($rows))
		{
			message($mm, "delete {$rows[$cursor]['name']}.hmm? [y/N]");
			$key = '';
			while ($key == '')
			{
				usleep(20000);
			...
			if (strtolower(substr($key,0,1)) === 'y')
				unlink($rows[$cursor]['file']);
		}
	}
}

function rename_map($old, $new)
{
	global $mm;
	$old_file = $mm['map_dir'].'/'.$old.'.hmm';
	$new_file = $mm['map_dir'].'/'.$new.'.hmm';

	if (file_exists($new_file))
	{
		message($mm, "$new.hmm already exists");
		return;
	}
	...
	foreach (glob($mm['map_dir'].'/*.hmm') as $file)
	{
		...
		$r = str_replace(["[[$old]]", "[[$old#"], ["[[$new]]", "[[$new#"], $c);
		if ($r !== $c)
		{
	...
	message($mm, "renamed, $touched files updated");
}
...
{
	echo "\033]2;".( $title == '' ? ' ' : 'hmx -- '.str_replace('.hmm','',basename($title)) )."\007";
}
...
if (!defined('HMX_TEST')) {
	...
	if (isset($mm['arguments']['filename']))
		load_map($mm['arguments']['filename']);
	else
		$mm['filename'] = '';
