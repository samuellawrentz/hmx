...
## Build and gates
`go build -o hmx .` · `go vet ./...` · `go test ./...` (unit + golden layout) · `bash test/tui.sh` (tmux smoke, 8 scenarios, `HMX_BIN` defaults to `./hmx`). Fixtures in `test/fixtures/` use real tabs. Deps: tcell/v2 and go-runewidth only; ask before adding one.
